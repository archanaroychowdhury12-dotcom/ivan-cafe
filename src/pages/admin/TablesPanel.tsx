...
import { Download, ExternalLink, Plus, Printer, QrCode, Trash2 } from 'lucide-react';
...
export default function TablesPanel() {
  ...
  const linkFor = (code: string) => `${origin}/menu?table=${code}`;
  ...
  return (
    <div className="space-y-5">
      ...
      <TableEditor table={editing} onClose={() => setEditing(null)} />
    ...
    <article className="overflow-hidden rounded-[26px] border border-line bg-paper shadow-card transition hover:shadow-lift">
      <div className="flex items-start gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            <ExternalLink size={11} /> Open guest menu
          </a>
        </div>
      </div>
...
function TableEditor({ table, onClose }: { table: CafeTable | null; onClose: () => void }) {
  const tables = useTables();
  const toast = useToast();
    ...
    <Sheet open={!!table} onClose={onClose} title="Table settings">
      <div className="space-y-4 px-5 py-5">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-line bg-paper p-4">
          <QrCode size={18} className="text-ember" />
          <p className="flex-1 text-[12px] text-mocha">
            The QR encodes <code className="text-ink">/menu?table={draft.code}</code> — regenerated instantly
